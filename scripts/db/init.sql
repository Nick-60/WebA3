-- Online Leave Management System - MySQL 初始化脚本
-- DB_NAME: leave_mgmt
-- 运行：mysql -uroot -pnick030201 < scripts/db/init.sql

SET NAMES utf8mb4;
CREATE DATABASE IF NOT EXISTS leave_mgmt CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE leave_mgmt;

-- 统一设置
SET @@session.sql_mode = REPLACE(@@sql_mode, 'ONLY_FULL_GROUP_BY', '');

-- 表：departments（先建以解决循环外键）
...
-- 后置添加循环外键：departments.manager_id -> users.id
...
-- 可选：插入审计日志示例（不影响验收）
INSERT INTO audit_logs(entity, entity_id, action, user_id, details)
VALUES ('users', (SELECT id FROM users WHERE username='emp'), 'CREATE', (SELECT id FROM users WHERE username='hr'), JSON_OBJECT('note','seed data'));
